...
export default function AuthModal() {
  ...
  const handleSignInSubmit = async (e) => {
    ...
    try {
      ...
      addToast(error.message || 'Unable to sign in. Check your credentials.', 'error');
    }
  };
  ...
  const handleVerifyOtp = (e) => {
    e.preventDefault();
    if (pendingUser && otpCode === '1234') {
      register(pendingUser).catch((error) => {
        addToast(error.message || 'Unable to create your account.', 'error');
      });
      setIsOtpStep(false);
      setPendingUser(null);
    } else {
      addToast('Enter the verification code 1234.', 'error');
    }
  };
